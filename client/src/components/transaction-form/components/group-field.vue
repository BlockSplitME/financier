...
const groupsNames = computed<string[]>(() =>
  props.groups
    .filter((group) => group.name !== CONSTANTS.DEFAULT_GROUP_NAME)
    .map((group) => group.name)
);
const isGroupNameEmpty = computed<boolean>(() =>
  Boolean(!(group.value.name ?? "").trim())
);
const isGroupNameChosen = computed<boolean>(
  () =>
    !isGroupNameEmpty.value &&
    group.value.name.trim() !== CONSTANTS.DEFAULT_GROUP_NAME
);
...
const onFocusedDescriptionField = (focused: boolean) => {
  if (!isGroupDescriptionReadonly.value)
    isGroupDescriptionReadonly.value = !focused;
};
...
onMounted(() => {
  document
    .querySelector(
      `.group-field__input-name-${props.label} > .v-input__control `
    )
